...
void StepperDriver::updateMicrosteps() { microsteps = 1; }
...
StepperMotor::~StepperMotor() { delete[] Driver; }
...
StepperJoint::StepperJoint(double numSteps, uint8_t pin_s, uint8_t pin_e,
                           uint8_t pin_d) {
  setDriverPins(pin_s, pin_e, pin_d);
  stepsPerRotation = numSteps;
  position = 0.0;
}
void StepperJoint::setHome() { position = 0.0; }
void StepperJoint::halt(bool stop) {
  if (stop) {
...
void StepperJoint::goToAngle(double theta, double rpm) {
  double dist =
      (theta - position) * (stepsPerRotation * Driver->microsteps) / (2 * PI);
...
unsigned long StepperJoint::rpmToMicros(double rpm) {
  return (1.0 / rpm) * (1000000.0 * 60.0) /
         (stepsPerRotation * Driver->microsteps);
}
